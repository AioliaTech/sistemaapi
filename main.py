...
def admin_redeploy_client(client_id: str, _auth=Depends(require_api_auth)):
    ...
    return {"message": f"Redeploy iniciado para '{client.name}'", "client_id": client_id}
...
def client_health(slug: str):
    ...
    if not client:
        raise HTTPException(status_code=404, detail=f"Cliente '{slug}' não encontrado")
    return {"status": "healthy", "client": slug, "timestamp": datetime.now().isoformat()}
    ...
    if tipo == "moto":
        ...
        for word in model_words:
            if len(word) >= 3 and word in MAPEAMENTO_CATEGORIAS:
                ...
                return JSONResponse(content={"modelo": modelo, "tipo": tipo, "categoria": categoria, "match_type": "partial_word"})
